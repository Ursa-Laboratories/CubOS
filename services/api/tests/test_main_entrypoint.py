"""Tests for the `python -m cubos_api` monorepo entrypoint."""
...
def test_main_warns_when_compiled_web_assets_are_missing(monkeypatch, tmp_path, caplog):
    from cubos_api import __main__ as api_main

    missing = tmp_path / "web" / "dist"
    monkeypatch.setattr(api_main, "FRONTEND_DIST", missing)
    monkeypatch.setattr(
        api_main,
        "CubOSSettings",
        lambda: type(
    ...
    assert observed["args"] == ("cubos_api.app:create_app",)
