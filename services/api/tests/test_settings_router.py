...
def restore_config_dir(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("CUBOS_SETTINGS_FILE", str(settings_file))
    monkeypatch.delenv("CUBOS_CONFIG_DIR", raising=False)
    yield settings_file
...
def test_get_settings_returns_config_dir(monkeypatch):
    with tempfile.TemporaryDirectory() as d:
        ...
        response = api_request(app, "GET", "/api/v1/settings")
...
def test_update_settings_accepts_config_dir():
    with tempfile.TemporaryDirectory() as d:
        ...
        response = api_request(app, "PUT", "/api/v1/settings", json={"config_dir": d})
...
def test_cubos_settings_loads_persisted_config_dir(restore_config_dir):
    with tempfile.TemporaryDirectory() as d:
...
def test_cubos_config_dir_env_wins_over_persisted_settings(monkeypatch, restore_config_dir, tmp_path):
    ...
    monkeypatch.setenv("CUBOS_CONFIG_DIR", str(env_dir))
...
def test_update_settings_rejects_invalid_path():
    ...
    response = api_request(app, "PUT", "/api/v1/settings", json={"config_dir": "/does/not/exist"})
...
def test_browse_directory_returns_selected_config_dir(monkeypatch):
    selected = "/tmp/cubos-configs"

    monkeypatch.setattr(
        "cubos_api.routers.settings.subprocess.run",
        lambda *args, **kwargs: CompletedProcess(args=args[0], returncode=0, stdout=f"{selected}\n"),
    )

    app = create_app()
    response = api_request(app, "POST", "/api/v1/settings/browse")
...
def test_browse_directory_wraps_picker_failures(monkeypatch):
    ...
    response = api_request(create_app(), "POST", "/api/v1/settings/browse")
