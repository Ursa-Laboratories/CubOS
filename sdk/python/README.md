...
`/api/v1/runs` backend used by the CubOS web interface.
...
history. This directory is the released `cubos-client` package inside the CubOS
monorepo, and its Git history includes the original PiCub sender and worker.
...
```bash
export CUBOS_API_TOKEN_FILE=/path/to/cub-api-token
cubos-send ...
```
...
```bash
export CUBOS_API_TOKEN='device-token'
cubos-send ...
```
