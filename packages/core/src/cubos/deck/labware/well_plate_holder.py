...
class WellPlateHolder(HolderLabware):
    ...
    well_plate_surface_height_from_bottom: float = Field(
        default=5.0,
        description=(
            "Vertical distance from holder bottom to the nested plate's "
            "calibrated well/rim surface."
        ),
    )

    @field_validator("well_plate_surface_height_from_bottom")
    def _validate_surface_height(cls, value: float) -> float:
        if value <= 0:
            raise ValueError(
    ...
    def _validate_surface_not_below_seat(self) -> "WellPlateHolder":
        ...
        if seat_height is not None and surface_height < seat_height:
            raise ValueError(
