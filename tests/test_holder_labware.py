...
def test_sharc_deck_nested_plate_surface_resolves_to_plate_rim_z():
    ...
    assert deck.resolve("plate_holder.plate.A1") == Coordinate3D(
        x=15.0, y=17.0, z=89.5,
    )
    assert deck.resolve("plate_holder.plate.A2") == Coordinate3D(
        x=15.0, y=26.0, z=89.5,
    )
    assert deck.resolve("plate_holder.plate.B1") == Coordinate3D(
