...
Use this map after reading `AGENTS.md`. Read only the source, docs, and tests needed for the task unless the change crosses shared interfaces, coordinate semantics, hardware motion, YAML schemas, or setup/protocol boundaries.
...
- `src/gantry/machine_geometry.py` — built-in fixed-structure AABBs per gantry family (for example, `cub_xl` right-rail guard). Not user-authored YAML; consumed by setup validation.
...
Gantry YAML requires top-level `gantry_type` (`cub` or `cub_xl`). For `cub_xl`, setup validation rejects protocols whose instrument point or known travel segment would hit the fixed right X-max rail. Setup bounds validation checks the concrete motion targets implied by the loaded protocol, not every physical geometry anchor or unused labware point on the deck.
